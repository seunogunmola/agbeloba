...
class Payments extends Admin_controller{
    ...
    function index($id=null,$hashed_account_number = NULL){
        ...
        $filter_year = "";
        $rules = array(
                'payment_month'=>array('field'=>'payment_month','label'=>'Payment Month','rules'=>'trim|required'),
                'payment_year'=>array('field'=>'payment_year','label'=>'Payment Year','rules'=>'trim|required'),
            );
        $this->form_validation->set_rules($rules);
        if($this->form_validation->run() == TRUE){
         $data = $this->input->post();
         $filter = array();
         if($data['payment_month'] != 'All'){
             $filter_month = $data['payment_month'];
         }
         if($data['payment_year'] != 'All'){
            $filter_year = $data['payment_year'];
         }
        }
         //GET PENDING ACCOUNTS
        $this->data['month'] = $current_month = !empty($filter_month) ? $filter_month:round(date('m'));
        $this->data['year'] = $current_year = !empty($filter_year)?$filter_year:date('Y');
        $this->data['current_month'] = !empty($filter_month)?$filter_month:date('M');
        $this->data['current_year'] = !empty($filter_year)?$filter_year:date('Y');
        // GET
        ...
        $this->db->group_by('t_referrer_relationships.parent_id');
        ...
        $this->db->having('count(t_referrer_relationships.account_number) > 0');
        $this->db->group_by('t_referrer_relationships.grand_parent_id');
    ...
    #GET ADDITIONAL SCRIPTS AND STYLES NEEDED FOR THE VIEW
        ...
        $this->data['subview'] = 'Administrator/Payments/pending_payments_page';
        $this->load->view('Administrator/_layout_main',$this->data);
    }
    function paid($id=null,$hashed_account_number = NULL){
        ...
        if($this->form_validation->run() == TRUE){
         ...
         $filter = array();
         if($data['payment_month'] != 'All'){
             $filter['t_payments.payment_month'] = $data['payment_month'];
         }
         if($data['payment_year'] != 'All'){
             $filter['t_payments.payment_year'] = $data['payment_year'];
         }
        }
        ...
        $this->db->select('t_payments.*,t_accounts.account_surname,t_accounts.account_othernames,t_accounts.account_title');
        $this->db->join('t_accounts','t_accounts.account_number = t_payments.account_number','inner');
        $this->db->order_by('payment_month DESC,payment_year DESC, payment_date DESC');
        $this->data['payments_data'] = $this->Payments_model->get();
        ...
        $this->data['subview'] = 'Administrator/Payments/paid_payments_page';
        $this->load->view('Administrator/_layout_main',$this->data);
    }
    ...
    function view_payment_details($hashed_account_number,$hashed_month,$hashed_year){
        if(isset($hashed_account_number) && isset($hashed_month) && isset($hashed_year)) {
            ...
            if(!count($this->data['level1_accounts_data']) && $this->data['level2_accounts_data']){
                ...
                $this->data['subview'] = 'Administrator/Payments/show_payment_details_page';
                $this->load->view('Administrator/_layout_main',$this->data);
            }
        ...
        if(isset($hashed_payment_id)) {
            ...
            $this->data['payment_data'] = $payment_data = $this->Payments_model->get($id = NULL,$single = TRUE);
                if(count($payment_data)){
                    ...
                    $this->data['subview'] = 'Administrator/Payments/payment_receipt_page';
                    $this->load->view('Administrator/_layout_main',$this->data);
                }
                else{
